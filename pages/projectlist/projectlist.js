		...
		swan.request({
			...
			success: function (res) {
				...
				if (res.statusCode == 200 && res.data.data.length > 0) {
					...
					var values = [];
					for (let i = 0; i < res.data.data.length; i++) {
						if (names.indexOf(res.data.data[i].name) < 0) names.push(res.data.data[i].name);
						if (values.indexOf(res.data.data[i].ftId) < 0 ) values.push(res.data.data[i].ftId);
					}
					_this.setData({
						tabTypes: names,
						tabTypeVals: values,
  ...
  /**   
	...
	leftChange: function (e) {
		...
		if (value < that.data.rightValue) {
			start_price = value;
			end_price = that.data.rightValue;
		}	else {
			start_price = that.data.rightValue;
			end_price = value;
		}
	...
	rightChange: function (e) {
		...
		if (value < that.data.leftValue) {
			start_price = value
			end_price = that.data.leftValue
		}	else {
			start_price = that.data.leftValue
			end_price = value
		}
		post.minPrice = minprice;
		post.maxPrice = maxprice;
	...
	submit: function() {
		...
		for (let i = 0; i < postcols.length; i++){
			if (post[postcols[i]] > 0 || post[postcols[i]] != ''){
